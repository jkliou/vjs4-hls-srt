...
gulp.task('js:videojs', function () {
    return browserify()
        .require(path.dirname(require.resolve('video.js')) + "/video.dev.js", {expose: 'video.js', fullPaths: false})
        .plugin(collapse)
        .bundle()
...
gulp.task('js:copy-videojs-srt.js', function () {
    return gulp.src(path.dirname(require.resolve('video.js')) + '/../../src/videosub-0.9.9.js')
        .pipe(gulp.dest('dist/js'));
});
gulp.task('js', [
    'js:videojs',
